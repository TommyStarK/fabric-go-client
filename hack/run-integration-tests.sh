...
docker ps -a | grep "peer0.org1.dummy.com-sacc-\(1.0\|2.0\)" | awk '{print $1}'| $XARGS docker rm -f;
docker images | grep "peer0.org1.dummy.com-sacc-\(1.0\|2.0\)" | awk '{print $3}'| $XARGS docker rmi -f;
